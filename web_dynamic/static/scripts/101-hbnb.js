...
function toggleReviews() {
  ...
  const reviewsToggleText = $('#toggle-reviews-text');

  if (reviewsToggleText.text() === 'Show Reviews') {
    // Fetch, parse, and display reviews
    fetchReviews().then((reviews) => {
      displayReviews(reviews);
      reviewsToggleText.text('Hide Reviews');
    });
  } else {
    // Hide reviews
    reviewsContainer.empty();
    reviewsToggleText.text('Show Reviews');
  }
}

function fetchReviews() {
  const REVIEWS_URL = `http://${HOST}:5001/api/v1/reviews/`;
  
  return $.get(REVIEWS_URL).then((data) => {
    return data.reviews;
  }).catch((error) => {
    console.error(error);
...
function displayReviews(reviews) {
  ...
  for (const review of reviews) {
    const reviewElement = $('<p>').text(review.text);
